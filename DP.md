...
int find(vector<int>& prices, int i, int k, bool buy,vector<vector<int>>& v) {
  ...
  // if we are buying then next time we will sell else next time
  // we will buy
  ...
  //+prices[i], because we now gain (i.e) sell our stock at rate of
  //prices[i]
    else{
...
31. Given an integer array nums, find the subarray with the largest sum, and return its sum.
